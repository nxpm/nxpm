...
export function validateConfig(config: ReleaseConfig): ValidatedConfig | false {
  ...
  if (!validated.version) {
    log(
      red('ERROR'),
      'Please provide the release version (like: 1.2.3-beta.4)'
    );
    return false;
  }
  ...
  if (!parseVersion(config.version).isValid) {
    log(
      red('ERROR'),
      'Please provide a valid release version (like: 1.2.3-beta.4)'
    );
    return false;
  }
...
export function validateWorkspace(
  ...
  if (!libs.length) {
    throw new Error(
      `No libraries found in nx workspace ${config.workspacePath}`
    );
  }
...
export function validatePackages(
  ...
  if (invalid.length) {
    ...
    if (!config.fix) {
      log(
        'Try running this command with the --fix flag to fix some common problems'
      );
    }
    return false;
  }
  ...
  const pkgFiles: string[] = workspace.packages
    .map((pkg) => {
      if (
        !pkg.architect?.options?.outputPath &&
        !pkg.architect?.options?.project
      ) {
  ...
  const foundPkgFiles = pkgFiles.map((pkgFile) => {
    ...
    if (!exists) {
      error(
        `Could not find ${pkgFile}. Make sure to build your packages before releasing`
      );
      return false;
    }
